 ...
 */
void connect_port_to_Ringer(LADSPA_Handle instance, unsigned long Port, LADSPA_Data * data_location)
{
	Ringer * ringer;
...
{
	/*
	 * allocate memory for Ringer_descriptor (it's just a pointer at this point).
	 * in other words create an actual LADSPA_Descriptor struct instance that
	 * Ringer_descriptor will point to.
	 */
	Ringer_descriptor = (LADSPA_Descriptor *) malloc(sizeof(LADSPA_Descriptor));
	...
	if (Ringer_descriptor)
	{
		 ...
		 * plugin, "Ringer" is fine. (NOTE: it must not have white spaces as per ladspa.h).
		...
		LADSPA_PortDescriptor * temp_descriptor_array;	// used for allocating and initailizing a
																		...
																		// an array of ints) since Ringer_descriptor->
		 ...
		 */
		temp_descriptor_array[RINGER_OUTPUT] = LADSPA_PORT_OUTPUT | LADSPA_PORT_AUDIO;
		...
		char ** temp_port_names;	// temporary local variable (which is a pointer to an array
											...
											// Ringer_descriptor->PortNames is a const char * const *.
		...
		*/
		...
		// set the name of the control port for the number of sample copies to be made
		...
		LADSPA_PortRangeHint * temp_hints;	// temporary local variable (pointer to a
														// PortRangeHint struct) since Ringer_descriptor->
		...
		* is pointed at.
		*/
		Ringer_descriptor->PortRangeHints = (const LADSPA_PortRangeHint *) temp_hints;
		 ...
		 * integers.
		 */
		temp_hints[RINGER_COPY_COUNT].HintDescriptor = (LADSPA_HINT_BOUNDED_BELOW
